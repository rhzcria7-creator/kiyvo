...
function cleanup() {
  ...
  blockedIPs.forEach((value, key) => {
    // Auto-desbloqueia após 45s a 25min dependendo da razão
    const blockTime = value.reason === 'bot_detected' ? 25 * 60_000 : value.reason === 'brute_force_auth' ? 15 * 60_000 : 45_000
    if (now - value.blockedAt > blockTime) blockedIPs.delete(key)
  })
}

// Respostas BONITAS de erro: redireciona páginas para /erro/limite; retorna JSON bonito para APIs
function buildBlockResponse(request: NextRequest, code: string, waitSeconds: number, isApi: boolean) {
  if (isApi) {
    return NextResponse.json(
      {
        error: code === 'bot_detected'
          ? 'Atividade suspeita detectada. Aguarde e tente novamente.'
          : code === 'banned'
            ? 'Seu acesso foi bloqueado temporariamente.'
            : 'Muitas requisições. Aguarde para tentar novamente.',
        code,
        retryAfter: waitSeconds,
        blockUntil: Date.now() + waitSeconds * 1000,
      },
      {
        status: code === 'banned' ? 403 : 429,
        headers: { 'Retry-After': String(waitSeconds) },
      },
    )
  }
  const url = new URL('/erro/limite', request.url)
...
const PUBLIC_PREFIXES = [
  ...
  '/loja/',       // lojas públicas de vendedores (qualquer visitante pode navegar)
  ...
  '/api/seller/', // Lojas públicas (qualquer visitante pode ver a loja/seller de um vendedor). O PATCH de perfil exige sessão no próprio handler.
...
function getSecurityHeaders(): Record<string, string> {
  return {
    ...
    'Content-Security-Policy': [
      "default-src 'self'",
      "script-src 'self' 'unsafe-inline' 'unsafe-eval' https://js.stripe.com https://cdn.jsdelivr.net https://www.gstatic.com https://apis.google.com https://accounts.google.com https://github.com",
      "style-src 'self' 'unsafe-inline' https://fonts.googleapis.com",
      "font-src 'self' https://fonts.gstatic.com data:",
      "img-src 'self' data: https: blob:",
      "connect-src 'self' https://api.stripe.com https://ytiyqkliojawihfnlwzo.supabase.co wss://ytiyqkliojawihfnlwzo.supabase.co https://generativelanguage.googleapis.com https://api.groq.com https://openrouter.ai https://integrate.api.nvidia.com https://api.duckduckgo.com https://identitytoolkit.googleapis.com https://securetoken.googleapis.com https://*.firebaseio.com https://www.googleapis.com https://github.com https://api.github.com",
      "frame-src https://js.stripe.com https://hooks.stripe.com https://connect-js.stripe.com https://accounts.google.com https://github.com",
...
export function middleware(request: NextRequest) {
  ...
  if (blockedIPs.has(ip)) {
    ...
    const elapsed = Date.now() - b.blockedAt
    const blockTime = b.reason === 'bot_detected' ? 25 * 60_000 : b.reason === 'brute_force_auth' ? 15 * 60_000 : 45_000
    const wait = Math.max(5, Math.ceil((blockTime - elapsed) / 1000))
  ...
  const fpLimit = checkRateLimit(`fp:${fingerprint}`, 120, 60000)
  if (!fpLimit.allowed) {
    return buildBlockResponse(request, 'rate_limit', 15, isApi)
  }
  ...
  // 5. Rate limit de APIs (60 req/min por rota — humano consegue)
  if (pathname.startsWith('/api/')) {
    const key = `api:${ip}:${pathname.split('/').slice(0, 4).join('/')}`
    const apiLimit = checkRateLimit(key, 60, 60000)
    if (!apiLimit.allowed) {
      return buildBlockResponse(request, 'rate_limit', 10, true)
    }
  }

  // 6. Rate limit de auth (10 tentativas em 5 min — evita brute mas não frustra humanos)
  const isAuthAttempt =
