// POST /api/agents/salespageminimalista
import { NextRequest, NextResponse } from 'next/server'
import { salespageminimalista } from '@/lib/agents/v10-monster'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await salespageminimalista(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'salespageminimalista')
      } catch { /* ignore */ }
    }
