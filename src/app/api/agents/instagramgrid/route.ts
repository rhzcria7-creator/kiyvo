// POST /api/agents/instagramgrid
import { NextRequest, NextResponse } from 'next/server'
import { instagramgrid } from '@/lib/agents/v10-monster'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await instagramgrid(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'instagramgrid')
      } catch { /* ignore */ }
    }
