// POST /api/agents/otimizadoranuncios
import { NextRequest, NextResponse } from 'next/server'
import { otimizadoranuncios } from '@/lib/agents/v101-boom'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await otimizadoranuncios(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'otimizadoranuncios')
      } catch { /* ignore */ }
    }
