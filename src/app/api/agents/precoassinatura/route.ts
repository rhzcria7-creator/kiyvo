// POST /api/agents/precoassinatura
import { NextRequest, NextResponse } from 'next/server'
import { runPrecificacaoAssinatura } from '@/lib/agents/v9-monetization'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await runPrecificacaoAssinatura(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'precoassinatura')
      } catch { /* ignore */ }
    }
