// POST /api/agents/scriptprejulgado
import { NextRequest, NextResponse } from 'next/server'
import { scriptprejulgado } from '@/lib/agents/v101-boom'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await scriptprejulgado(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'scriptprejulgado')
      } catch { /* ignore */ }
    }
