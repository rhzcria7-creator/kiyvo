// POST /api/agents/roteiroreels
import { NextRequest, NextResponse } from 'next/server'
import { roteiroreels } from '@/lib/agents/v101-boom'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await roteiroreels(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'roteiroreels')
      } catch { /* ignore */ }
    }
