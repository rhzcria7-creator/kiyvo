// POST /api/agents/calculadoratributos
import { NextRequest, NextResponse } from 'next/server'
import { calculadoratributos } from '@/lib/agents/v101-boom'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await calculadoratributos(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'calculadoratributos')
      } catch { /* ignore */ }
    }
