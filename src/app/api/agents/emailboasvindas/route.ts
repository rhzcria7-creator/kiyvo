// POST /api/agents/emailboasvindas
import { NextRequest, NextResponse } from 'next/server'
import { runEmailBoasVindas } from '@/lib/agents/emailboasvindas'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await runEmailBoasVindas(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'emailboasvindas')
      } catch { /* ignore */ }
    }
