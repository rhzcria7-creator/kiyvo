// POST /api/agents/clientepravida
import { NextRequest, NextResponse } from 'next/server'
import { runClientePraVida } from '@/lib/agents/clientepravida'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await runClientePraVida(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'clientepravida')
      } catch { /* ignore */ }
    }
