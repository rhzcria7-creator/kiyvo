// POST /api/agents/seolocalpages
import { NextRequest, NextResponse } from 'next/server'
import { seolocalpages } from '@/lib/agents/v10-monster'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await seolocalpages(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'seolocalpages')
      } catch { /* ignore */ }
    }
