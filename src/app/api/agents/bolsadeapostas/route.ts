// POST /api/agents/bolsadeapostas
import { NextRequest, NextResponse } from 'next/server'
import { bolsadeapostas } from '@/lib/agents/v10-monster'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await bolsadeapostas(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'bolsadeapostas')
      } catch { /* ignore */ }
    }
