// POST /api/agents/carrinhoabandonado
import { NextRequest, NextResponse } from 'next/server'
import { runCarrinhoAbandonado } from '@/lib/agents/carrinhoabandonado'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await runCarrinhoAbandonado(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'carrinhoabandonado')
      } catch { /* ignore */ }
    }
