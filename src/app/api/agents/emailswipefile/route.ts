// POST /api/agents/emailswipefile
import { NextRequest, NextResponse } from 'next/server'
import { emailswipefile } from '@/lib/agents/v10-monster'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await emailswipefile(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'emailswipefile')
      } catch { /* ignore */ }
    }
