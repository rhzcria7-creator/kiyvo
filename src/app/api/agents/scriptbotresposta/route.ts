// POST /api/agents/scriptbotresposta
import { NextRequest, NextResponse } from 'next/server'
import { runScriptBotResposta } from '@/lib/agents/scriptbotresposta'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await runScriptBotResposta(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'scriptbotresposta')
      } catch { /* ignore */ }
    }
