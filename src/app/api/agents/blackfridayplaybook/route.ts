// POST /api/agents/blackfridayplaybook
import { NextRequest, NextResponse } from 'next/server'
import { blackfridayplaybook } from '@/lib/agents/v10-monster'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await blackfridayplaybook(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'blackfridayplaybook')
      } catch { /* ignore */ }
    }
