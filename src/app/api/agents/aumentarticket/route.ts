// POST /api/agents/aumentarticket
import { NextRequest, NextResponse } from 'next/server'
import { runTicketMedioAumento } from '@/lib/agents/v92-monetization'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await runTicketMedioAumento(input, { userId, plan } as any)
    if (userId !== 'anon') {
      try { const u = getUsage(userId); u.copiesHoje = (u.copiesHoje||0)+1; setUsage(userId, u); trackUsage(userId, 'aumentarticket') } catch {}
    }
