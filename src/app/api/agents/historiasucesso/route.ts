// POST /api/agents/historiasucesso
import { NextRequest, NextResponse } from 'next/server'
import { historiasucesso } from '@/lib/agents/v101-boom'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await historiasucesso(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'historiasucesso')
      } catch { /* ignore */ }
    }
