// POST /api/agents/cancelasaver
import { NextRequest, NextResponse } from 'next/server'
import { runCancelaSaver } from '@/lib/agents/cancelasaver'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await runCancelaSaver(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'cancelasaver')
      } catch { /* ignore */ }
    }
