// POST /api/agents/calculadorapl
import { NextRequest, NextResponse } from 'next/server'
import { calculadorapl } from '@/lib/agents/v101-boom'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await calculadorapl(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'calculadorapl')
      } catch { /* ignore */ }
    }
