// POST /api/agents/upgradeplano
import { NextRequest, NextResponse } from 'next/server'
import { runPlanoUpgradeOferta } from '@/lib/agents/v9-monetization'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await runPlanoUpgradeOferta(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'upgradeplano')
      } catch { /* ignore */ }
    }
