// POST /api/agents/contratoprestserv
import { NextRequest, NextResponse } from 'next/server'
import { contratoprestserv } from '@/lib/agents/v101-boom'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await contratoprestserv(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'contratoprestserv')
      } catch { /* ignore */ }
    }
