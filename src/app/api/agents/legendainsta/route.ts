// POST /api/agents/legendainsta
import { NextRequest, NextResponse } from 'next/server'
import { legendainsta } from '@/lib/agents/v101-boom'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await legendainsta(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'legendainsta')
      } catch { /* ignore */ }
    }
