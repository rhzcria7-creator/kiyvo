// POST /api/agents/bumpcriador
import { NextRequest, NextResponse } from 'next/server'
import { runCriadorBump } from '@/lib/agents/v92-monetization'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await runCriadorBump(input, { userId, plan } as any)
    if (userId !== 'anon') {
      try { const u = getUsage(userId); u.copiesHoje = (u.copiesHoje||0)+1; setUsage(userId, u); trackUsage(userId, 'bumpcriador') } catch {}
    }
