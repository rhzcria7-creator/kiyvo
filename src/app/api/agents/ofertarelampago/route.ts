// POST /api/agents/ofertarelampago
import { NextRequest, NextResponse } from 'next/server'
import { runOfertaRelampago } from '@/lib/agents/ofertarelampago'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await runOfertaRelampago(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'ofertarelampago')
      } catch { /* ignore */ }
    }
