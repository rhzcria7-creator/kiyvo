// POST /api/agents/pitchvenda
import { NextRequest, NextResponse } from 'next/server'
import { runPitchVenda } from '@/lib/agents/v93-monetization'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await runPitchVenda(input, { userId, plan: getPlanForUser({ plano: userPlano as any }) } as any)
    if (userId !== 'anon') { try { const u = getUsage(userId); u.copiesHoje=(u.copiesHoje||0)+1; setUsage(userId,u); trackUsage(userId,'pitchvenda') } catch {} }
