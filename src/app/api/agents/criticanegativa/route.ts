// POST /api/agents/criticanegativa
import { NextRequest, NextResponse } from 'next/server'
import { criticanegativa } from '@/lib/agents/v101-boom'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await criticanegativa(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'criticanegativa')
      } catch { /* ignore */ }
    }
