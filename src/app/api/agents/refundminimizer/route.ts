// POST /api/agents/refundminimizer
import { NextRequest, NextResponse } from 'next/server'
import { runRefundMinimizer } from '@/lib/agents/refundminimizer'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await runRefundMinimizer(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'refundminimizer')
      } catch { /* ignore */ }
    }
