// POST /api/agents/sniperlancamento
import { NextRequest, NextResponse } from 'next/server'
import { runScriptLancamento } from '@/lib/agents/v9-monetization'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await runScriptLancamento(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'sniperlancamento')
      } catch { /* ignore */ }
    }
