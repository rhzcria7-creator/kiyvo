// POST /api/agents/tiktokscripts
import { NextRequest, NextResponse } from 'next/server'
import { tiktokscripts } from '@/lib/agents/v10-monster'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await tiktokscripts(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'tiktokscripts')
      } catch { /* ignore */ }
    }
