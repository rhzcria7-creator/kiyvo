// POST /api/agents/launchecklist30d
import { NextRequest, NextResponse } from 'next/server'
import { launchecklist30d } from '@/lib/agents/v10-monster'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await launchecklist30d(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'launchecklist30d')
      } catch { /* ignore */ }
    }
