// POST /api/agents/whatsappvendas
import { NextRequest, NextResponse } from 'next/server'
import { runWhatsAppVendas } from '@/lib/agents/whatsappvendas'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await runWhatsAppVendas(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'whatsappvendas')
      } catch { /* ignore */ }
    }
