// POST /api/agents/saquesimulator
import { NextRequest, NextResponse } from 'next/server'
import { runSaqueSimulator } from '@/lib/agents/v9-monetization'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await runSaqueSimulator(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'saquesimulator')
      } catch { /* ignore */ }
    }
