// POST /api/agents/checkoutkick
import { NextRequest, NextResponse } from 'next/server'
import { checkoutkick } from '@/lib/agents/v101-boom'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await checkoutkick(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'checkoutkick')
      } catch { /* ignore */ }
    }
