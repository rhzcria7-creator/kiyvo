// POST /api/agents/precificacaointeligente
import { NextRequest, NextResponse } from 'next/server'
import { runPrecificacaoInteligente } from '@/lib/agents/precificacaointeligente'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await runPrecificacaoInteligente(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'precificacaointeligente')
      } catch { /* ignore */ }
    }
