// POST /api/agents/canvaprompts
import { NextRequest, NextResponse } from 'next/server'
import { canvaprompts } from '@/lib/agents/v10-monster'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await canvaprompts(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'canvaprompts')
      } catch { /* ignore */ }
    }
