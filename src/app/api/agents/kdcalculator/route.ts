// POST /api/agents/kdcalculator
import { NextRequest, NextResponse } from 'next/server'
import { runKDCalculator } from '@/lib/agents/kdcalculator'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await runKDCalculator(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'kdcalculator')
      } catch { /* ignore */ }
    }
