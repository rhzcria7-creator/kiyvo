// POST /api/agents/whatsappfunnel
import { NextRequest, NextResponse } from 'next/server'
import { whatsappfunnel } from '@/lib/agents/v10-monster'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await whatsappfunnel(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'whatsappfunnel')
      } catch { /* ignore */ }
    }
