// POST /api/agents/podcastguestpitch
import { NextRequest, NextResponse } from 'next/server'
import { podcastguestpitch } from '@/lib/agents/v10-monster'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await podcastguestpitch(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'podcastguestpitch')
      } catch { /* ignore */ }
    }
