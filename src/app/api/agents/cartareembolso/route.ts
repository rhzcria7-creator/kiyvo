// POST /api/agents/cartareembolso
import { NextRequest, NextResponse } from 'next/server'
import { cartareembolso } from '@/lib/agents/v101-boom'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await cartareembolso(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'cartareembolso')
      } catch { /* ignore */ }
    }
