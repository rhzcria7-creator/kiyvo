// POST /api/agents/pitchdeck
import { NextRequest, NextResponse } from 'next/server'
import { runPitchDeck } from '@/lib/agents/v93-monetization'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await runPitchDeck(input, { userId, plan: getPlanForUser({ plano: userPlano as any }) } as any)
    if (userId !== 'anon') { try { const u = getUsage(userId); u.copiesHoje=(u.copiesHoje||0)+1; setUsage(userId,u); trackUsage(userId,'pitchdeck') } catch {} }
