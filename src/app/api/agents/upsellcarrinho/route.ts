// POST /api/agents/upsellcarrinho
import { NextRequest, NextResponse } from 'next/server'
import { runUpsellCarrinho } from '@/lib/agents/v9-monetization'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await runUpsellCarrinho(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'upsellcarrinho')
      } catch { /* ignore */ }
    }
