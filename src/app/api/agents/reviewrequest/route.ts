// POST /api/agents/reviewrequest
import { NextRequest, NextResponse } from 'next/server'
import { reviewrequest } from '@/lib/agents/v10-monster'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await reviewrequest(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'reviewrequest')
      } catch { /* ignore */ }
    }
