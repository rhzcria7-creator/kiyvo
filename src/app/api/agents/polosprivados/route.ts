// POST /api/agents/polosprivados
import { NextRequest, NextResponse } from 'next/server'
import { polosprivados } from '@/lib/agents/v101-boom'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await polosprivados(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'polosprivados')
      } catch { /* ignore */ }
    }
