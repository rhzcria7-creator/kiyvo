// POST /api/agents/otosequencia
import { NextRequest, NextResponse } from 'next/server'
import { runOTOSequencia } from '@/lib/agents/v92-monetization'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await runOTOSequencia(input, { userId, plan } as any)
    if (userId !== 'anon') {
      try { const u = getUsage(userId); u.copiesHoje = (u.copiesHoje||0)+1; setUsage(userId, u); trackUsage(userId, 'otosequencia') } catch {}
    }
