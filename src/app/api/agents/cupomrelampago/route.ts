// POST /api/agents/cupomrelampago
import { NextRequest, NextResponse } from 'next/server'
import { runCupomRelampago } from '@/lib/agents/v9-monetization'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await runCupomRelampago(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'cupomrelampago')
      } catch { /* ignore */ }
    }
