// POST /api/agents/vslgenerator
import { NextRequest, NextResponse } from 'next/server'
import { vslgenerator } from '@/lib/agents/v10-monster'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await vslgenerator(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'vslgenerator')
      } catch { /* ignore */ }
    }
