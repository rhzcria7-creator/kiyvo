// POST /api/agents/kdpointscampaign
import { NextRequest, NextResponse } from 'next/server'
import { kdpointscampaign } from '@/lib/agents/v10-monster'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await kdpointscampaign(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'kdpointscampaign')
      } catch { /* ignore */ }
    }
