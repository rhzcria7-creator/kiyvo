// POST /api/agents/bundlecriador
import { NextRequest, NextResponse } from 'next/server'
import { runBundleCriador } from '@/lib/agents/bundlecriador'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await runBundleCriador(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'bundlecriador')
      } catch { /* ignore */ }
    }
