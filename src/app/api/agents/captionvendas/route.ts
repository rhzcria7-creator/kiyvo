// POST /api/agents/captionvendas
import { NextRequest, NextResponse } from 'next/server'
import { runCaptionVendas } from '@/lib/agents/captionvendas'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await runCaptionVendas(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'captionvendas')
      } catch { /* ignore */ }
    }
