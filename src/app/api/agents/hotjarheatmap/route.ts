// POST /api/agents/hotjarheatmap
import { NextRequest, NextResponse } from 'next/server'
import { hotjarheatmap } from '@/lib/agents/v10-monster'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await hotjarheatmap(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'hotjarheatmap')
      } catch { /* ignore */ }
    }
