// POST /api/agents/emailrecuperacao
import { NextRequest, NextResponse } from 'next/server'
import { emailrecuperacao } from '@/lib/agents/v101-boom'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await emailrecuperacao(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'emailrecuperacao')
      } catch { /* ignore */ }
    }
