// POST /api/agents/provaosocialpro
import { NextRequest, NextResponse } from 'next/server'
import { runProvaSocialPro } from '@/lib/agents/provaosocialpro'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await runProvaSocialPro(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'provaosocialpro')
      } catch { /* ignore */ }
    }
