// POST /api/agents/calendarioconteudo
import { NextRequest, NextResponse } from 'next/server'
import { calendarioconteudo } from '@/lib/agents/v101-boom'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await calendarioconteudo(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'calendarioconteudo')
      } catch { /* ignore */ }
    }
