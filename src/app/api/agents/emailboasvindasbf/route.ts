// POST /api/agents/emailboasvindasbf
import { NextRequest, NextResponse } from 'next/server'
import { emailboasvindasbf } from '@/lib/agents/v101-boom'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await emailboasvindasbf(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'emailboasvindasbf')
      } catch { /* ignore */ }
    }
