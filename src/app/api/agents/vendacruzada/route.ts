// POST /api/agents/vendacruzada
import { NextRequest, NextResponse } from 'next/server'
import { runVendaCruzada } from '@/lib/agents/v9-monetization'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await runVendaCruzada(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'vendacruzada')
      } catch { /* ignore */ }
    }
