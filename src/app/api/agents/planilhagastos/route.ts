// POST /api/agents/planilhagastos
import { NextRequest, NextResponse } from 'next/server'
import { planilhagastos } from '@/lib/agents/v101-boom'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await planilhagastos(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'planilhagastos')
      } catch { /* ignore */ }
    }
