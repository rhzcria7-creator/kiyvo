// POST /api/agents/whatsappsequencia
import { NextRequest, NextResponse } from 'next/server'
import { whatsappsequencia } from '@/lib/agents/v101-boom'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await whatsappsequencia(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'whatsappsequencia')
      } catch { /* ignore */ }
    }
