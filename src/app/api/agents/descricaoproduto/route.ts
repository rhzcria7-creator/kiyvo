// POST /api/agents/descricaoproduto
import { NextRequest, NextResponse } from 'next/server'
import { descricaoproduto } from '@/lib/agents/v101-boom'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await descricaoproduto(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'descricaoproduto')
      } catch { /* ignore */ }
    }
