// POST /api/agents/afiliadorpro
import { NextRequest, NextResponse } from 'next/server'
import { runAfiliadorPro } from '@/lib/agents/afiliadorpro'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await runAfiliadorPro(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'afiliadorpro')
      } catch { /* ignore */ }
    }
