// POST /api/agents/leadmagnetpro
import { NextRequest, NextResponse } from 'next/server'
import { leadmagnetpro } from '@/lib/agents/v10-monster'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await leadmagnetpro(input, { userId, plan } as any)
    ...
    if (userId !== 'anon') {
      try {
        ...
        setUsage(userId, u)
        trackUsage(userId, 'leadmagnetpro')
      } catch { /* ignore */ }
    }
