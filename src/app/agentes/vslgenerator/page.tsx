'use client'
// Página do agente: Gerador de VSL
import { useState } from 'react'
import { motion } from 'framer-motion'
import { Video, CheckCircle, Copy } from 'lucide-react'
...
export default function AgentePage() {
  ...
  const formId = 'form-vslgenerator'
  ...
  async function handleGerar(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const res = await fetch('/api/agents/vslgenerator', {
        method: 'POST',
  ...
  const icon = <Video className="w-7 h-7" />

  return (
    <AgentShell
      titulo="Gerador de VSL"
      tagline="Script completo de VSL (Video Sales Letter) de 10 minutos com alto poder de conversão."
      icone={icon}
