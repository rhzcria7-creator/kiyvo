'use client'
// Página auto-gerada para o agente Lead Magnet (KIYVO v9.0)
...
export default function Page() {
  const [input, setInput] = useState<any>({ nicho: '', publico: '' })
  ...
  async function gerar() {
    ...
    setResult(null)
    try {
      const res = await fetch('/api/agents/leadmagnet', {
        method: 'POST',
  ...
  return (
    <AgentShell
      titulo="Lead Magnet"
      tagline="Iscas éticas que convertem"
      icone={<span className="text-2xl">{ICON}</span>}
      cor="bg-gradient-to-br from-cyan-500 to-cyan-700"
      onGerar={gerar}
      loading={loading}
    ...
    >
      ...
      <Field label="Público alvo">
        <input type="text" value={input.publico || ''} onChange={e => setInput({ ...input, publico: e.target.value })} placeholder="" className={inputClass} />
      </Field>
    </AgentShell>
  )
}
