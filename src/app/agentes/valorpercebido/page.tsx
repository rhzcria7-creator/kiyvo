...
export default function Page() {
  const [input, setInput] = useState<any>({ preco: undefined, valorBonusTotal: undefined, horasConteudo: undefined, horasSessao: undefined })
  const [loading, setLoading] = useState(false)
  const [result, setResult] = useState<any>(null)
  const [copied, setCopied] = useState(false)
  async function gerar() {
    setLoading(true); setResult(null)
    try { const res = await fetch('/api/agents/valorpercebido',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(input)}); setResult(await res.json()) } finally { setLoading(false) }
  }
  ...
  return (<AgentShell titulo="Valor Percebido" tagline="Calcula valor x preço da oferta" icone={<span className="text-2xl">💎</span>} cor="from-violet-500 to-purple-700" onGerar={gerar} loading={loading} labelBotao="Gerar com IA" output={result?.ok?(<motion.div initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} className="space-y-4"><div className="flex items-center justify-between"><div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 text-xs font-bold">✅ Pronto</div><button onClick={copiar} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-slate-200 dark:border-slate-700 text-xs font-semibold">{copied?<Check className="w-3.5 h-3.5"/>:<Copy className="w-3.5 h-3.5"/>}{copied?'Copiado!':'Copiar'}</button></div>{rv(result.data)}</motion.div>):result?.error?(<div className="py-12 text-center"><p className="text-red-500 font-semibold text-sm">{result.error}</p></div>):null} ><Field label="Preço (R$)">
        <input type="number" value={input.preco ?? ''} onChange={e => setInput({ ...input, preco: e.target.valueAsNumber || e.target.value })} placeholder="" className={inputClass} />
      </Field>
      <Field label="Valor dos bônus (R$)">
        <input type="number" value={input.valorBonusTotal ?? ''} onChange={e => setInput({ ...input, valorBonusTotal: e.target.valueAsNumber || e.target.value })} placeholder="0" className={inputClass} />
      </Field>
      <Field label="Horas de conteúdo">
        <input type="number" value={input.horasConteudo ?? ''} onChange={e => setInput({ ...input, horasConteudo: e.target.valueAsNumber || e.target.value })} placeholder="10" className={inputClass} />
      </Field>
      <Field label="Horas de sessão">
        <input type="number" value={input.horasSessao ?? ''} onChange={e => setInput({ ...input, horasSessao: e.target.valueAsNumber || e.target.value })} placeholder="0" className={inputClass} />
      </Field></AgentShell>)
}
