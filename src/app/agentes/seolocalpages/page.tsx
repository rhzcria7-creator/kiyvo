'use client'
// Página do agente: SEO Local (cidades)
...
export default function AgentePage() {
  ...
  const formId = 'form-seolocalpages'
  ...
  async function handleGerar(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (input.cidadesStr) { input.cidades = String(input.cidadesStr).split(',').map((s: string) => s.trim()).filter(Boolean); delete input.cidadesStr }
      const res = await fetch('/api/agents/seolocalpages', {
        method: 'POST',
  ...
  const icon = <MapPin className="w-7 h-7" />

  return (
    <AgentShell
      titulo="SEO Local (cidades)"
      tagline="Gera páginas SEO otimizadas por cidade + nicho para rankear no Google Brasil."
      icone={icon}
