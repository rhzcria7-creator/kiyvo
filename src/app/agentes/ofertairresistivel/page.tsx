'use client'
// Página do agente: Criador de Oferta Irresistível
...
export default function AgentePage() {
  ...
  const formId = 'form-ofertairresistivel'
  ...
  async function handleGerar(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const res = await fetch('/api/agents/ofertairresistivel', {
        method: 'POST',
  ...
  const icon = <Zap className="w-7 h-7" />

  return (
    <AgentShell
      titulo="Criador de Oferta Irresistível"
      tagline="Transforme produtos comuns em ofertas que ninguém recusa."
      icone={icon}
      cor="bg-gradient-to-br from-fuchsia-500 to-purple-700"
    ...
    >
      <form id={formId} onSubmit={handleGerar} className="space-y-4">
        <Field label="Seu produto principal"><input className={inputClass} name="produto" placeholder="" required /></Field>
        <Field label="Preço atual (R$)"><input className={inputClass} type="number" step="0.01" name="preco" defaultValue="" required /></Field>
        <Field label="Custo por venda (R$)"><input className={inputClass} type="number" step="0.01" name="custo" defaultValue="" required /></Field>
        <Field label="Preço do concorrente (R$)"><input className={inputClass} type="number" step="0.01" name="concorrente" defaultValue="" required /></Field>
