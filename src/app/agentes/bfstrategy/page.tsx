'use client'
// Página auto-gerada — Black Friday Playbook
...
export default function Page() {
  ...
  async function gerar() {
    ...
    try {
      const res = await fetch('/api/agents/bfstrategy', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(input) })
  ...
  return (
    <AgentShell
      titulo="Black Friday Playbook" tagline="Estratégia completa de BF"
      icone={<span className="text-2xl">🖤</span>} cor="from-slate-800 to-black"
    ...
    >
      <Field label="Preço normal (R$)">
        <input type="number" value={input.precoNormal ?? ''} onChange={e => setInput({ ...input, precoNormal: e.target.valueAsNumber || Number(e.target.value) || e.target.value })} placeholder="" className={inputClass} />
      </Field>
      <Field label="Custo por unidade (R$)">
        <input type="number" value={input.custo ?? ''} onChange={e => setInput({ ...input, custo: e.target.valueAsNumber || Number(e.target.value) || e.target.value })} placeholder="0" className={inputClass} />
      </Field>
    </AgentShell>
  )
}
