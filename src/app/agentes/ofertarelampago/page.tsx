'use client'
// Página auto-gerada para o agente Oferta Relâmpago (KIYVO v9.0)
...
export default function Page() {
  const [input, setInput] = useState<any>({ produtoNome: '', precoNormal: undefined, desconto: undefined })
  ...
  async function gerar() {
    ...
    setResult(null)
    try {
      const res = await fetch('/api/agents/ofertarelampago', {
        method: 'POST',
  ...
  return (
    <AgentShell
      ...
      tagline="Promoção de 24h com tudo o que precisa"
    ...
    >
      ...
      <Field label="Desconto %">
        <input type="number" value={input.desconto || ''} onChange={e => setInput({ ...input, desconto: e.target.value })} placeholder="50" className={inputClass} />
      </Field>
    </AgentShell>
  )
}
