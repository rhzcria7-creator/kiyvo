'use client'
// Página auto-gerada — MRR Calculator
...
export default function Page() {
  const [input, setInput] = useState<any>({ assinantes: undefined, valorMensal: undefined, churnPercent: undefined })
  ...
  async function gerar() {
    ...
    try {
      const res = await fetch('/api/agents/mrrcalc', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(input) })
  ...
  return (
    <AgentShell
      titulo="MRR Calculator" tagline="Receita recorrente de assinatura"
      icone={<span className="text-2xl">📈</span>} cor="from-cyan-500 to-blue-700"
    ...
    >
      <Field label="Assinantes atuais">
        <input type="number" value={input.assinantes ?? ''} onChange={e => setInput({ ...input, assinantes: e.target.valueAsNumber || Number(e.target.value) || e.target.value })} placeholder="" className={inputClass} />
      </Field>
      <Field label="Valor mensal (R$)">
        <input type="number" value={input.valorMensal ?? ''} onChange={e => setInput({ ...input, valorMensal: e.target.valueAsNumber || Number(e.target.value) || e.target.value })} placeholder="" className={inputClass} />
      </Field>
      <Field label="Churn mensal %">
        <input type="number" value={input.churnPercent ?? ''} onChange={e => setInput({ ...input, churnPercent: e.target.valueAsNumber || Number(e.target.value) || e.target.value })} placeholder="5" className={inputClass} />
      </Field>
    </AgentShell>
  )
}
