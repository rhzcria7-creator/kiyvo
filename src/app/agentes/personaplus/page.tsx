'use client'
// Página do agente: Persona Profunda
import { useState } from 'react'
import { motion } from 'framer-motion'
import { User, CheckCircle, Copy } from 'lucide-react'
...
export default function AgentePage() {
  ...
  async function handleGerar(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const res = await fetch('/api/agents/personaplus', {
        method: 'POST',
  ...
  const icon = <User className="w-7 h-7" />

  return (
    <AgentShell
      titulo="Persona Profunda"
      tagline="Persona com dores, sonhos e frases reais que a pessoa fala."
      icone={icon}
    ...
    >
      <form id={formId} onSubmit={handleGerar} className="space-y-4">
        ...
        <Field label="Faixa etária"><input className={inputClass} name="faixaIdade" placeholder="" required /></Field>
        <Field label="Gênero"><select className={selectClass} name="genero" required>
        <option value="misto">Misto</option>
        <option value="feminino">Feminino</option>
        <option value="masculino">Masculino</option>
      </select></Field>
        <Field label="Renda mensal"><select className={selectClass} name="renda" required>
        <option value="ate2k">Até R$2k</option>
        <option value="2a5k">R$2-5k</option>
        <option value="5a10k">R$5-10k</option>
        <option value="10kmais">R$10k+</option>
      </select></Field>
