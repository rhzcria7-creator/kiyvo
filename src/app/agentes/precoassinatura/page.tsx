'use client'
// Página auto-gerada — Preço de Assinatura
...
export default function Page() {
  const [input, setInput] = useState<any>({ valorUnico: undefined, nomeProduto: undefined })
  ...
  async function gerar() {
    ...
    try {
      const res = await fetch('/api/agents/precoassinatura', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(input) })
  ...
  return (
    <AgentShell
      titulo="Preço de Assinatura" tagline="Mensal/trimestral/anual justo"
      icone={<span className="text-2xl">🔁</span>} cor="from-blue-500 to-indigo-700"
    ...
    >
      <Field label="Valor do produto à vista (R$)">
        <input type="number" value={input.valorUnico ?? ''} onChange={e => setInput({ ...input, valorUnico: e.target.valueAsNumber || Number(e.target.value) || e.target.value })} placeholder="97" className={inputClass} />
      </Field>
      <Field label="Nome do produto">
        <input type="text" value={input.nomeProduto ?? ''} onChange={e => setInput({ ...input, nomeProduto: e.target.valueAsNumber || Number(e.target.value) || e.target.value })} placeholder="" className={inputClass} />
      </Field>
    </AgentShell>
  )
}
