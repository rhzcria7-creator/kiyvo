'use client'
// Página auto-gerada — Cupom Relâmpago
...
export default function Page() {
  const [input, setInput] = useState<any>({ descontoPercent: undefined, minimoValor: undefined, horas: undefined })
  ...
  async function gerar() {
    ...
    try {
      const res = await fetch('/api/agents/cupomrelampago', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(input) })
  ...
  return (
    <AgentShell
      titulo="Cupom Relâmpago" tagline="Cupom KD urgente"
      icone={<span className="text-2xl">⚡</span>} cor="from-yellow-500 to-orange-600"
    ...
    >
      <Field label="Desconto %">
        <input type="number" value={input.descontoPercent ?? ''} onChange={e => setInput({ ...input, descontoPercent: e.target.valueAsNumber || Number(e.target.value) || e.target.value })} placeholder="15" className={inputClass} />
      </Field>
      <Field label="Mínimo (R$)">
        <input type="number" value={input.minimoValor ?? ''} onChange={e => setInput({ ...input, minimoValor: e.target.valueAsNumber || Number(e.target.value) || e.target.value })} placeholder="47" className={inputClass} />
      </Field>
      <Field label="Validade horas">
        <input type="number" value={input.horas ?? ''} onChange={e => setInput({ ...input, horas: e.target.valueAsNumber || Number(e.target.value) || e.target.value })} placeholder="2" className={inputClass} />
      </Field>
    </AgentShell>
  )
}
