...
export default function Page() {
  const [input, setInput] = useState<any>({ preco: undefined, plano: undefined })
  ...
  async function gerar() {
    ...
    try {
      const res = await fetch('/api/agents/cashbackcalc', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(input) })
  ...
  return (
    <AgentShell
      titulo="Calculadora Cashback KD" tagline="Calcula KD Points da compra"
      icone={<span className="text-2xl">🪙</span>} cor="from-yellow-400 to-amber-600"
    ...
    >
      ...
      <Field label="Plano">
          <select value={input.plano ?? ''} onChange={e => setInput({ ...input, plano: e.target.value })} className={selectClass}>
