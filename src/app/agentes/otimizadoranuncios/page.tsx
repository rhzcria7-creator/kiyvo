'use client'
// Página do agente: Auditor de Anúncios Meta/Google
...
export default function AgentePage() {
  ...
  const formId = 'form-otimizadoranuncios'
  ...
  async function handleGerar(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const res = await fetch('/api/agents/otimizadoranuncios', {
        method: 'POST',
  ...
  const icon = <Search className="w-7 h-7" />

  return (
    <AgentShell
      titulo="Auditor de Anúncios Meta/Google"
      tagline="Descubra por que seus anúncios não estão convertendo e como baixar o CPA."
      icone={icon}
    ...
    >
      <form id={formId} onSubmit={handleGerar} className="space-y-4">
        ...
        <Field label="Produto/oferta"><input className={inputClass} name="produto" placeholder="" required /></Field>
        <Field label="Preço (R$)"><input className={inputClass} type="number" step="0.01" name="valor" defaultValue="" required /></Field>
        <Field label="CPC atual (R$)"><input className={inputClass} type="number" step="0.01" name="cpcAtual" defaultValue="" required /></Field>
        <Field label="CPA atual (R$)"><input className={inputClass} type="number" step="0.01" name="cpaAtual" defaultValue="" required /></Field>
        <Field label="CTR atual (%)"><input className={inputClass} type="number" step="0.01" name="ctrAtual" defaultValue="" required /></Field>
        <Field label="Copy atual"><textarea className={textareaClass} name="copyAtual" rows={4} required></textarea></Field>
