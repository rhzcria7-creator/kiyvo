...
export default function Page() {
  const [input, setInput] = useState<any>({ receitaMensal: undefined })
  ...
  async function gerar() {
    ...
    try {
      const res = await fetch('/api/agents/profitfirst', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(input) })
  ...
  return (
    <AgentShell
      titulo="Profit First (Lucro Primeiro)" tagline="50/30/10/10 adaptado pra Brasil"
      icone={<span className="text-2xl">🏦</span>} cor="from-green-500 to-teal-700"
    ...
    >
      <Field label="Receita mensal (R$)">
        <input type="number" value={input.receitaMensal ?? ''} onChange={e => setInput({ ...input, receitaMensal: e.target.valueAsNumber || e.target.value })} placeholder="5000" className={inputClass} />
      </Field>
    </AgentShell>
  )
}
