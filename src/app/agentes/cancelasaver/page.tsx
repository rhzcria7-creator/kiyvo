'use client'
// Página auto-gerada para o agente Cancela Saver (KIYVO v9.0)
...
export default function Page() {
  const [input, setInput] = useState<any>({ plano: '', preco: undefined })
  ...
  async function gerar() {
    ...
    setResult(null)
    try {
      const res = await fetch('/api/agents/cancelasaver', {
        method: 'POST',
  ...
  return (
    <AgentShell
      titulo="Cancela Saver"
      tagline="Fluxo que salva cancelamento de assinatura"
      icone={<span className="text-2xl">{ICON}</span>}
      cor="bg-gradient-to-br from-emerald-600 to-green-800"
      onGerar={gerar}
      loading={loading}
    ...
    >
      <Field label="Nome do plano">
        <input type="text" value={input.plano || ''} onChange={e => setInput({ ...input, plano: e.target.value })} placeholder="" className={inputClass} />
      </Field>
