'use client'
// Página auto-gerada — Oferta de Upgrade
...
export default function Page() {
  const [input, setInput] = useState<any>({ planoAtual: undefined, planoDesejado: undefined, valorPlano: undefined })
  ...
  async function gerar() {
    ...
    try {
      const res = await fetch('/api/agents/upgradeplano', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(input) })
  ...
  return (
    <AgentShell
      titulo="Oferta de Upgrade" tagline="Argumento para subir de plano"
      icone={<span className="text-2xl">🚀</span>} cor="from-emerald-500 to-teal-700"
    ...
    >
      <Field label="Plano atual">
          <select value={input.planoAtual || ''} onChange={e => setInput({ ...input, planoAtual: e.target.value })} className={selectClass}>
      ...
      <Field label="Plano desejado">
          <select value={input.planoDesejado || ''} onChange={e => setInput({ ...input, planoDesejado: e.target.value })} className={selectClass}>
            <option value="">Selecione...</option>
            <option value="plus">plus</option>
            <option value="pro">pro</option>
      ...
      <Field label="Preço do plano desejado (R$)">
        <input type="number" value={input.valorPlano ?? ''} onChange={e => setInput({ ...input, valorPlano: e.target.valueAsNumber || Number(e.target.value) || e.target.value })} placeholder="" className={inputClass} />
      </Field>
    </AgentShell>
  )
}
