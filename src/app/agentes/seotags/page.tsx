'use client'
// Página auto-gerada para o agente SEO Tags (KIYVO v9.0)
...
export default function Page() {
  const [input, setInput] = useState<any>({ produtoNome: '', categoria: '' })
  ...
  async function gerar() {
    ...
    setResult(null)
    try {
      const res = await fetch('/api/agents/seotags', {
        method: 'POST',
  ...
  return (
    <AgentShell
      titulo="SEO Tags"
      tagline="Title + description otimizados"
      icone={<span className="text-2xl">{ICON}</span>}
      cor="bg-gradient-to-br from-blue-600 to-indigo-800"
      onGerar={gerar}
      loading={loading}
    ...
    >
      ...
      <Field label="Categoria">
        <input type="text" value={input.categoria || ''} onChange={e => setInput({ ...input, categoria: e.target.value })} placeholder="" className={inputClass} />
      </Field>
    </AgentShell>
  )
}
