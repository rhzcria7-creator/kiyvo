'use client'
// Página auto-gerada para o agente Prova Social Pro (KIYVO v9.0)
...
export default function Page() {
  const [input, setInput] = useState<any>({ produtoNome: '', tipo: '' })
  ...
  async function gerar() {
    ...
    setResult(null)
    try {
      const res = await fetch('/api/agents/provaosocialpro', {
        method: 'POST',
  ...
  return (
    <AgentShell
      titulo="Prova Social Pro"
      tagline="Depoimentos que convencem de verdade"
      icone={<span className="text-2xl">{ICON}</span>}
      cor="bg-gradient-to-br from-pink-500 to-pink-700"
      onGerar={gerar}
      loading={loading}
