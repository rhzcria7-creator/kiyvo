'use client'
// Página auto-gerada para o agente CheckoutMax (KIYVO v9.0)
...
export default function Page() {
  const [input, setInput] = useState<any>({ produtoNome: '', preco: undefined, nicho: '' })
  ...
  async function gerar() {
    ...
    setResult(null)
    try {
      const res = await fetch('/api/agents/checkoutmax', {
        method: 'POST',
  ...
  return (
    <AgentShell
      titulo="CheckoutMax"
      tagline="Otimize seu checkout com gatilhos psicológicos"
    ...
    >
      <Field label="Nome do produto">
        <input type="text" value={input.produtoNome || ''} onChange={e => setInput({ ...input, produtoNome: e.target.value })} placeholder="Ex: Curso de Marketing Digital" className={inputClass} />
      </Field>
      <Field label="Preço (R$)">
        <input type="number" value={input.preco || ''} onChange={e => setInput({ ...input, preco: e.target.value })} placeholder="97" className={inputClass} />
      </Field>
      <Field label="Nicho">
        <input type="text" value={input.nicho || ''} onChange={e => setInput({ ...input, nicho: e.target.value })} placeholder="Ex: marketing digital" className={inputClass} />
      </Field>
    </AgentShell>
  )
}
