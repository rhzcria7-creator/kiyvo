'use client'
// Página do agente: Calculadora PLR/Direitos de Revenda
...
export default function AgentePage() {
  ...
  const formId = 'form-calculadorapl'
  ...
  async function handleGerar(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const res = await fetch('/api/agents/calculadorapl', {
        method: 'POST',
  ...
  const icon = <BookOpen className="w-7 h-7" />

  return (
    <AgentShell
      titulo="Calculadora PLR/Direitos de Revenda"
      tagline="Preço ideal de PLR/MRR para vender revenda com lucro."
      icone={icon}
      cor="bg-gradient-to-br from-violet-500 to-purple-800"
    ...
    >
      <form id={formId} onSubmit={handleGerar} className="space-y-4">
        <Field label="Tipo de direito"><select className={selectClass} name="tipo" required>
        <option value="plr">PLR</option>
        <option value="mrr">MRR</option>
        <option value="rr">Resell Rights</option>
        <option value="white_label">White Label</option>
      </select></Field>
        <Field label="Custo de criação (R$)"><input className={inputClass} type="number" step="0.01" name="precoCriacao" defaultValue="" required /></Field>
        <Field label="Preço final ao consumidor (R$)"><input className={inputClass} type="number" step="0.01" name="precoVendaFinal" defaultValue="" required /></Field>
        <Field label="Cópias a vender"><input className={inputClass} type="number" step="0.01" name="numeroCopias" defaultValue="100" required /></Field>
