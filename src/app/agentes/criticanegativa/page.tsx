'use client'
// Página do agente: Resposta a Críticas e Reclamações
import { useState } from 'react'
import { motion } from 'framer-motion'
import { MessageCircleWarning, CheckCircle, Copy } from 'lucide-react'
...
export default function AgentePage() {
  ...
  const formId = 'form-criticanegativa'
  ...
  async function handleGerar(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const res = await fetch('/api/agents/criticanegativa', {
        method: 'POST',
  ...
  const icon = <MessageCircleWarning className="w-7 h-7" />

  return (
    <AgentShell
      titulo="Resposta a Críticas e Reclamações"
      tagline="Transforme críticas em oportunidades de mostrar profissionalismo."
      icone={icon}
    ...
    >
      <form id={formId} onSubmit={handleGerar} className="space-y-4">
        <Field label="Reclamação do cliente"><textarea className={textareaClass} name="reclamacao" rows={4} required></textarea></Field>
        ...
        <Field label="Nome da sua marca"><input className={inputClass} name="marca" placeholder="" required /></Field>
        <Field label="Política de reembolso"><select className={selectClass} name="politica" required>
        <option value="7dias">7 dias</option>
        <option value="garantia30">30 dias</option>
        <option value="suporte">Suporte prioritário</option>
      </select></Field>
