...
export default function Page() {
  const [input, setInput] = useState<any>({ produtoNome: undefined, dor: undefined, sonho: undefined })
  ...
  async function gerar() {
    ...
    try {
      const res = await fetch('/api/agents/scriptspn', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(input) })
  ...
  return (
    <AgentShell
      titulo="Script Venda SPN" tagline="Sofrimento → Ponte → Nova Vida"
      icone={<span className="text-2xl">🎭</span>} cor="from-violet-500 to-indigo-700"
    ...
    >
      ...
      <Field label="Sonho do cliente">
        <input type="text" value={input.sonho ?? ''} onChange={e => setInput({ ...input, sonho: e.target.valueAsNumber || e.target.value })} placeholder="" className={inputClass} />
      </Field>
    </AgentShell>
  )
}
