'use client'
// Página do agente: Checkout Kick Booster
...
export default function AgentePage() {
  ...
  const formId = 'form-checkoutkick'
  ...
  async function handleGerar(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const res = await fetch('/api/agents/checkoutkick', {
        method: 'POST',
  ...
  return (
    <AgentShell
      titulo="Checkout Kick Booster"
      tagline="Checklist de checkout com +20% de conversão garantida."
      icone={icon}
    ...
    >
      <form id={formId} onSubmit={handleGerar} className="space-y-4">
        ...
        <option value="kiyvo">KIYVO</option>
      </select></Field>
        ...
        <Field label="Conversão atual (%)"><input className={inputClass} type="number" step="0.01" name="taxaAtual" defaultValue="" required /></Field>
        <Field label="Elementos já existentes"><textarea className={textareaClass} name="elementos" rows={4} required></textarea></Field>
