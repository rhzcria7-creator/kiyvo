'use client'
// Página auto-gerada — Simulador de Saque
...
export default function Page() {
  const [input, setInput] = useState<any>({ valorSaque: undefined })
  ...
  async function gerar() {
    ...
    try {
      const res = await fetch('/api/agents/saquesimulator', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(input) })
  ...
  return (
    <AgentShell
      titulo="Simulador de Saque" tagline="Veja quanto chega na conta"
      icone={<span className="text-2xl">💸</span>} cor="from-green-500 to-emerald-700"
    ...
    >
      <Field label="Valor do saque (R$)">
        <input type="number" value={input.valorSaque ?? ''} onChange={e => setInput({ ...input, valorSaque: e.target.valueAsNumber || Number(e.target.value) || e.target.value })} placeholder="100" className={inputClass} />
      </Field>
    </AgentShell>
  )
}
