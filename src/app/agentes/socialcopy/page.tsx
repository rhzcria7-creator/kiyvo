'use client'
// Página auto-gerada para o agente Social Copy (KIYVO v9.0)
...
export default function Page() {
  const [input, setInput] = useState<any>({ tema: '' })
  ...
  async function gerar() {
    ...
    setResult(null)
    try {
      const res = await fetch('/api/agents/socialcopy', {
        method: 'POST',
  ...
  return (
    <AgentShell
      titulo="Social Copy"
      tagline="Posts para X, Instagram, LinkedIn"
      icone={<span className="text-2xl">{ICON}</span>}
      cor="bg-gradient-to-br from-red-400 to-pink-600"
      onGerar={gerar}
      loading={loading}
    ...
    >
      <Field label="Tema">
        <input type="text" value={input.tema || ''} onChange={e => setInput({ ...input, tema: e.target.value })} placeholder="" className={inputClass} />
      </Field>
    </AgentShell>
  )
}
