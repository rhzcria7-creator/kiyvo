...
export default function Page() {
  const [input, setInput] = useState<any>({ preco: undefined, meioPagamento: undefined, planoVendedor: undefined, comissaoAfiliado: undefined, custoProduto: undefined, cac: undefined })
  ...
  async function gerar() {
    ...
    try {
      const res = await fetch('/api/agents/custosreais', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(input) })
  ...
  return (
    <AgentShell
      titulo="Custos Reais de Venda" tagline="Todos os custos abertos (gateway+plataforma+impostos)"
      icone={<span className="text-2xl">🧾</span>} cor="from-slate-600 to-slate-900"
    ...
    >
      ...
      <Field label="Meio de pagamento">
          <select value={input.meioPagamento ?? ''} onChange={e => setInput({ ...input, meioPagamento: e.target.value })} className={selectClass}>
      ...
      <Field label="Seu plano">
          <select value={input.planoVendedor ?? ''} onChange={e => setInput({ ...input, planoVendedor: e.target.value })} className={selectClass}>
            <option value="">Selecione...</option>
            <option value="free">free</option>
            <option value="plus">plus</option>
            <option value="pro">pro</option>
            <option value="vendor_pro">vendor_pro</option>
          </select>
        </Field>
      <Field label="Comissão afiliado %">
        <input type="number" value={input.comissaoAfiliado ?? ''} onChange={e => setInput({ ...input, comissaoAfiliado: e.target.valueAsNumber || e.target.value })} placeholder="0" className={inputClass} />
      </Field>
      <Field label="Custo do produto (R$)">
        <input type="number" value={input.custoProduto ?? ''} onChange={e => setInput({ ...input, custoProduto: e.target.valueAsNumber || e.target.value })} placeholder="0" className={inputClass} />
      </Field>
      <Field label="CAC/custo de anúncio (R$)">
        <input type="number" value={input.cac ?? ''} onChange={e => setInput({ ...input, cac: e.target.valueAsNumber || e.target.value })} placeholder="0" className={inputClass} />
      </Field>
    </AgentShell>
  )
}
