'use client'
// Página do agente: Pitch para Podcast/YouTube
import { useState } from 'react'
import { motion } from 'framer-motion'
import { Mic, CheckCircle, Copy } from 'lucide-react'
...
export default function AgentePage() {
  ...
  const formId = 'form-podcastguestpitch'
  ...
  async function handleGerar(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const res = await fetch('/api/agents/podcastguestpitch', {
        method: 'POST',
  ...
  const icon = <Mic className="w-7 h-7" />

  return (
    <AgentShell
      titulo="Pitch para Podcast/YouTube"
      tagline="Templates de email para ser convidado em podcasts e ganhar autoridade no nicho."
      icone={icon}
      cor="bg-gradient-to-br from-red-500 to-rose-700"
