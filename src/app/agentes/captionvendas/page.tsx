'use client'
// Página auto-gerada para o agente Caption de Vendas (KIYVO v9.0)
...
export default function Page() {
  const [input, setInput] = useState<any>({ produtoNome: '', nicho: '' })
  ...
  async function gerar() {
    ...
    setResult(null)
    try {
      const res = await fetch('/api/agents/captionvendas', {
        method: 'POST',
  ...
  return (
    <AgentShell
      titulo="Caption de Vendas"
      tagline="Legendas de Reels/TikTok que vendem"
      icone={<span className="text-2xl">{ICON}</span>}
      cor="bg-gradient-to-br from-rose-500 to-rose-700"
      onGerar={gerar}
      loading={loading}
