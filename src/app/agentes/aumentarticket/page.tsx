...
export default function Page() {
  const [input, setInput] = useState<any>({ ticketAtual: undefined })
  ...
  async function gerar() {
    ...
    try {
      const res = await fetch('/api/agents/aumentarticket', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(input) })
  ...
  return (
    <AgentShell
      titulo="Aumentar Ticket Médio" tagline="10 estratégias de AOV com ganho%"
      icone={<span className="text-2xl">📈</span>} cor="from-indigo-500 to-blue-700"
    ...
    >
      <Field label="Ticket médio atual (R$)">
        <input type="number" value={input.ticketAtual ?? ''} onChange={e => setInput({ ...input, ticketAtual: e.target.valueAsNumber || e.target.value })} placeholder="" className={inputClass} />
      </Field>
    </AgentShell>
  )
}
