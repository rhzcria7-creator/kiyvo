'use client'
// Página auto-gerada para o agente Método Hero (KIYVO v9.0)
...
export default function Page() {
  const [input, setInput] = useState<any>({ nicho: '', resultado: '', dias: undefined })
  ...
  async function gerar() {
    ...
    setResult(null)
    try {
      const res = await fetch('/api/agents/metodohero', {
        method: 'POST',
  ...
  return (
    <AgentShell
      titulo="Método Hero"
      tagline="Crie nome e promessa do seu método"
      icone={<span className="text-2xl">{ICON}</span>}
      cor="bg-gradient-to-br from-sky-500 to-sky-700"
      onGerar={gerar}
      loading={loading}
    ...
    >
      ...
      <Field label="Resultado entregue">
        <input type="text" value={input.resultado || ''} onChange={e => setInput({ ...input, resultado: e.target.value })} placeholder="" className={inputClass} />
      </Field>
      <Field label="Prazo em dias">
        <input type="number" value={input.dias || ''} onChange={e => setInput({ ...input, dias: e.target.value })} placeholder="21" className={inputClass} />
      </Field>
    </AgentShell>
  )
}
