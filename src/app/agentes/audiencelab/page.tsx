'use client'
// Página do agente: Audience Lab — Públicos Lucrativos
import { useState } from 'react'
import { motion } from 'framer-motion'
import { Target, CheckCircle, Copy } from 'lucide-react'
...
export default function AgentePage() {
  ...
  async function handleGerar(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const res = await fetch('/api/agents/audiencelab', {
        method: 'POST',
  ...
  const icon = <Target className="w-7 h-7" />

  return (
    <AgentShell
      titulo="Audience Lab — Públicos Lucrativos"
      tagline="Gere 20 públicos segmentados que realmente compram."
      icone={icon}
      cor="bg-gradient-to-br from-purple-500 to-indigo-700"
    ...
    >
      <form id={formId} onSubmit={handleGerar} className="space-y-4">
        ...
        <Field label="Faixa de preço"><select className={selectClass} name="preco" required>
        <option value="ate50">Até R$50</option>
        <option value="50a200">R$50 a R$200</option>
        <option value="200a1000">R$200 a R$1000</option>
        <option value="1000mais">R$1000+</option>
      </select></Field>
