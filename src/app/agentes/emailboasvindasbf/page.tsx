'use client'
// Página do agente: Sequência Black Friday Completa
import { useState } from 'react'
import { motion } from 'framer-motion'
import { Flame, CheckCircle, Copy } from 'lucide-react'
...
export default function AgentePage() {
  ...
  const formId = 'form-emailboasvindasbf'
  ...
  async function handleGerar(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const res = await fetch('/api/agents/emailboasvindasbf', {
        method: 'POST',
  ...
  const icon = <Flame className="w-7 h-7" />

  return (
    <AgentShell
      titulo="Sequência Black Friday Completa"
      tagline="7 emails de BF com urgência progressiva que vende MUITO."
      icone={icon}
      cor="bg-gradient-to-br from-black to-gray-800"
    ...
    >
      <form id={formId} onSubmit={handleGerar} className="space-y-4">
        <Field label="Produto em promoção"><input className={inputClass} name="produto" placeholder="" required /></Field>
        <Field label="Preço normal (R$)"><input className={inputClass} type="number" step="0.01" name="precoNormal" defaultValue="" required /></Field>
        <Field label="Preço Black Friday (R$)"><input className={inputClass} type="number" step="0.01" name="precoBF" defaultValue="" required /></Field>
        <Field label="Desconto (%)"><input className={inputClass} type="number" step="0.01" name="desconto" defaultValue="60" required /></Field>
        <Field label="Público"><select className={selectClass} name="publico" required>
        <option value="leads_frios">Leads frios</option>
        <option value="clientes">Clientes antigos</option>
        <option value="carrinho_abandonado">Abandonadores</option>
      </select></Field>
