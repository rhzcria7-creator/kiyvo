'use client'
// Página do agente: FAQ que quebra 20 objeções
...
export default function AgentePage() {
  ...
  async function handleGerar(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const res = await fetch('/api/agents/faqproduto', {
        method: 'POST',
  ...
  const icon = <HelpCircle className="w-7 h-7" />

  return (
    <AgentShell
      titulo="FAQ que quebra 20 objeções"
      tagline="Perguntas e respostas que acabam com as dúvidas na hora."
      icone={icon}
      cor="bg-gradient-to-br from-slate-500 to-slate-700"
    ...
    >
      <form id={formId} onSubmit={handleGerar} className="space-y-4">
        ...
        <Field label="Dias de garantia"><input className={inputClass} type="number" step="0.01" name="garantia" defaultValue="7" required /></Field>
