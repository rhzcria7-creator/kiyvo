'use client'
// Página auto-gerada para o agente Viral Loop (KIYVO v9.0)
...
export default function Page() {
  const [input, setInput] = useState<any>({ produtoNome: '', recompensa: '' })
  ...
  async function gerar() {
    ...
    setResult(null)
    try {
      const res = await fetch('/api/agents/viralloop', {
        method: 'POST',
  ...
  return (
    <AgentShell
      titulo="Viral Loop"
      tagline="Crescimento por indicação ético"
      icone={<span className="text-2xl">{ICON}</span>}
      cor="bg-gradient-to-br from-pink-500 to-purple-700"
      onGerar={gerar}
      loading={loading}
    ...
    >
      ...
      <Field label="Recompensa">
        <input type="text" value={input.recompensa || ''} onChange={e => setInput({ ...input, recompensa: e.target.value })} placeholder="R$50 em crédito" className={inputClass} />
      </Field>
    </AgentShell>
  )
}
