'use client'
// Página auto-gerada para o agente Refund Minimizer (KIYVO v9.0)
...
export default function Page() {
  const [input, setInput] = useState<any>({ produtoNome: '' })
  ...
  async function gerar() {
    ...
    setResult(null)
    try {
      const res = await fetch('/api/agents/refundminimizer', {
        method: 'POST',
  ...
  return (
    <AgentShell
      titulo="Refund Minimizer"
      tagline="Reduza reembolsos sem bloquear clientes"
      icone={<span className="text-2xl">{ICON}</span>}
      cor="bg-gradient-to-br from-emerald-500 to-emerald-700"
      onGerar={gerar}
      loading={loading}
