'use client'
// Página auto-gerada para o agente Afiliador Pro (KIYVO v9.0)
...
export default function Page() {
  const [input, setInput] = useState<any>({ produtoNome: '', preco: undefined, comissao: undefined })
  ...
  async function gerar() {
    ...
    setResult(null)
    try {
      const res = await fetch('/api/agents/afiliadorpro', {
        method: 'POST',
  ...
  return (
    <AgentShell
      titulo="Afiliador Pro"
      tagline="Programa de afiliados magnético"
      icone={<span className="text-2xl">{ICON}</span>}
      cor="bg-gradient-to-br from-amber-500 to-amber-700"
      onGerar={gerar}
      loading={loading}
    ...
    >
      <Field label="Produto">
        <input type="text" value={input.produtoNome || ''} onChange={e => setInput({ ...input, produtoNome: e.target.value })} placeholder="" className={inputClass} />
      </Field>
      <Field label="Preço (R$)">
        <input type="number" value={input.preco || ''} onChange={e => setInput({ ...input, preco: e.target.value })} placeholder="" className={inputClass} />
      </Field>
      <Field label="Comissão %">
        <input type="number" value={input.comissao || ''} onChange={e => setInput({ ...input, comissao: e.target.value })} placeholder="50" className={inputClass} />
      </Field>
    </AgentShell>
  )
}
