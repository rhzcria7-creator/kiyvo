'use client'
// Página do agente: Calculadora de Tributos PF/PJ
import { useState } from 'react'
import { motion } from 'framer-motion'
import { Calculator, CheckCircle, Copy } from 'lucide-react'
...
export default function AgentePage() {
  ...
  const formId = 'form-calculadoratributos'
  ...
  async function handleGerar(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const res = await fetch('/api/agents/calculadoratributos', {
        method: 'POST',
  ...
  const icon = <Calculator className="w-7 h-7" />

  return (
    <AgentShell
      titulo="Calculadora de Tributos PF/PJ"
      tagline="Calcule IRPF, CSLL, PIS/COFINS, ISS, Simples Nacional e lucro real."
      icone={icon}
    ...
    >
      <form id={formId} onSubmit={handleGerar} className="space-y-4">
        <Field label="Regime tributário"><select className={selectClass} name="regime" required>
        <option value="pf">Pessoa Física</option>
        <option value="simples_nacional">PJ Simples Nacional</option>
        <option value="lucro_presumido">PJ Lucro Presumido</option>
        <option value="lucro_real">PJ Lucro Real</option>
      </select></Field>
        <Field label="Faturamento mensal (R$)"><input className={inputClass} type="number" step="0.01" name="faturamento" defaultValue="15000" required /></Field>
        <Field label="Custos mensais (R$)"><input className={inputClass} type="number" step="0.01" name="custos" defaultValue="3000" required /></Field>
        <Field label="Estado"><input className={inputClass} name="estado" placeholder="" required /></Field>
