'use client'
// Página auto-gerada — Boas-Vindas Primeira Compra
...
export default function Page() {
  const [input, setInput] = useState<any>({ valorCompra: undefined })
  ...
  async function gerar() {
    ...
    try {
      const res = await fetch('/api/agents/boasvindas', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(input) })
  ...
  return (
    <AgentShell
      titulo="Boas-Vindas Primeira Compra" tagline="Mensagem pós-compra com bônus"
      icone={<span className="text-2xl">🎉</span>} cor="from-pink-500 to-fuchsia-700"
    ...
    >
      <Field label="Valor da 1ª compra (R$)">
        <input type="number" value={input.valorCompra ?? ''} onChange={e => setInput({ ...input, valorCompra: e.target.valueAsNumber || Number(e.target.value) || e.target.value })} placeholder="" className={inputClass} />
      </Field>
    </AgentShell>
  )
}
