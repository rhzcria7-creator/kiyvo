'use client'
// Página do agente: Checklist Lançamento 30 dias
...
export default function AgentePage() {
  ...
  const formId = 'form-launchecklist30d'
  ...
  async function handleGerar(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const res = await fetch('/api/agents/launchecklist30d', {
        method: 'POST',
  ...
  const icon = <Rocket className="w-7 h-7" />

  return (
    <AgentShell
      titulo="Checklist Lançamento 30 dias"
      tagline="Checklist completo de lançamento de produto digital em 30 dias."
      icone={icon}
      cor="bg-gradient-to-br from-indigo-600 to-violet-700"
