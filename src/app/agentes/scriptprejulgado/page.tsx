'use client'
// Página do agente: Script Pré-Julgado de Oferta
import { useState } from 'react'
import { motion } from 'framer-motion'
import { Bomb, CheckCircle, Copy } from 'lucide-react'
...
export default function AgentePage() {
  ...
  const formId = 'form-scriptprejulgado'
  ...
  async function handleGerar(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const res = await fetch('/api/agents/scriptprejulgado', {
        method: 'POST',
  ...
  const icon = <Bomb className="w-7 h-7" />

  return (
    <AgentShell
      titulo="Script Pré-Julgado de Oferta"
      tagline="Páginas que fazem o cliente dizer SIM 9 vezes antes de ver o preço."
      icone={icon}
      cor="bg-gradient-to-br from-red-600 to-rose-800"
    ...
    >
      <form id={formId} onSubmit={handleGerar} className="space-y-4">
        ...
        <Field label="Dor principal do cliente"><input className={inputClass} name="dor" placeholder="" required /></Field>
