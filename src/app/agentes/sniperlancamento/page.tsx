'use client'
// Página auto-gerada — Script Lançamento 7 dias
...
export default function Page() {
  ...
  async function gerar() {
    ...
    try {
      const res = await fetch('/api/agents/sniperlancamento', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(input) })
  ...
  return (
    <AgentShell
      titulo="Script Lançamento 7 dias" tagline="Cronograma completo de lançamento"
      icone={<span className="text-2xl">🎯</span>} cor="from-violet-500 to-purple-700"
    ...
    >
      ...
      <Field label="Preço lançamento (R$)">
        <input type="number" value={input.preco ?? ''} onChange={e => setInput({ ...input, preco: e.target.valueAsNumber || Number(e.target.value) || e.target.value })} placeholder="" className={inputClass} />
      </Field>
    </AgentShell>
  )
}
