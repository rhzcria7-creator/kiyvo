'use client'
// Página auto-gerada para o agente KD Calculator (KIYVO v9.0)
...
export default function Page() {
  const [input, setInput] = useState<any>({ valorCompra: undefined, plano: '' })
  ...
  async function gerar() {
    ...
    setResult(null)
    try {
      const res = await fetch('/api/agents/kdcalculator', {
        method: 'POST',
  ...
  return (
    <AgentShell
      titulo="KD Calculator"
      tagline="Calcule cashback e KD Points"
    ...
    >
      <Field label="Valor da compra (R$)">
        <input type="number" value={input.valorCompra || ''} onChange={e => setInput({ ...input, valorCompra: e.target.value })} placeholder="" className={inputClass} />
      </Field>
      <Field label="Plano">
