'use client'
// Página auto-gerada para o agente Bundle Criador (KIYVO v9.0)
...
export default function Page() {
  const [input, setInput] = useState<any>({ produto1: '', preco1: undefined, produto2: '', preco2: undefined })
  ...
  async function gerar() {
    ...
    setResult(null)
    try {
      const res = await fetch('/api/agents/bundlecriador', {
        method: 'POST',
  ...
  return (
    <AgentShell
      titulo="Bundle Criador"
      tagline="Combos que aumentam ticket médio"
      icone={<span className="text-2xl">{ICON}</span>}
      cor="bg-gradient-to-br from-orange-500 to-orange-700"
      onGerar={gerar}
      loading={loading}
    ...
    >
      <Field label="Produto principal">
        <input type="text" value={input.produto1 || ''} onChange={e => setInput({ ...input, produto1: e.target.value })} placeholder="" className={inputClass} />
      </Field>
      <Field label="Preço prod. principal">
        <input type="number" value={input.preco1 || ''} onChange={e => setInput({ ...input, preco1: e.target.value })} placeholder="" className={inputClass} />
      </Field>
      <Field label="Complementar">
        <input type="text" value={input.produto2 || ''} onChange={e => setInput({ ...input, produto2: e.target.value })} placeholder="" className={inputClass} />
      </Field>
      <Field label="Preço complementar">
        <input type="number" value={input.preco2 || ''} onChange={e => setInput({ ...input, preco2: e.target.value })} placeholder="" className={inputClass} />
      </Field>
    </AgentShell>
  )
}
