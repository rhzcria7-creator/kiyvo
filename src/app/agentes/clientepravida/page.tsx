'use client'
// Página auto-gerada para o agente Cliente pra Vida (KIYVO v9.0)
...
export default function Page() {
  const [input, setInput] = useState<any>({ produtoNome: '', ticketMedio: undefined })
  ...
  async function gerar() {
    ...
    setResult(null)
    try {
      const res = await fetch('/api/agents/clientepravida', {
        method: 'POST',
  ...
  return (
    <AgentShell
      titulo="Cliente pra Vida"
      tagline="Retenha e aumente LTV"
      icone={<span className="text-2xl">{ICON}</span>}
      cor="bg-gradient-to-br from-fuchsia-500 to-fuchsia-700"
      onGerar={gerar}
      loading={loading}
    ...
    >
      ...
      <Field label="Ticket médio (R$)">
        <input type="number" value={input.ticketMedio || ''} onChange={e => setInput({ ...input, ticketMedio: e.target.value })} placeholder="" className={inputClass} />
      </Field>
    </AgentShell>
  )
}
