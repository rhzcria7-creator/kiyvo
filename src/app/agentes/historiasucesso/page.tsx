'use client'
// Página do agente: História de Sucesso/Cliente
...
export default function AgentePage() {
  ...
  const formId = 'form-historiasucesso'
  ...
  async function handleGerar(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const res = await fetch('/api/agents/historiasucesso', {
        method: 'POST',
  ...
  const icon = <Trophy className="w-7 h-7" />

  return (
    <AgentShell
      titulo="História de Sucesso/Cliente"
      tagline="Cases emocionantes para página de vendas e depoimentos."
      icone={icon}
      cor="bg-gradient-to-br from-yellow-400 to-amber-600"
    ...
    >
      <form id={formId} onSubmit={handleGerar} className="space-y-4">
        <Field label="Perfil do cliente"><input className={inputClass} name="cliente" placeholder="" required /></Field>
        <Field label="Seu produto"><input className={inputClass} name="produto" placeholder="" required /></Field>
        <Field label="Situação ANTES"><textarea className={textareaClass} name="antes" rows={4} required></textarea></Field>
        <Field label="Resultado DEPOIS"><textarea className={textareaClass} name="depois" rows={4} required></textarea></Field>
        <Field label="Tempo de transformação"><input className={inputClass} name="tempo" placeholder="" required /></Field>
