'use client'
// Página auto-gerada para o agente Carrinho Abandonado (KIYVO v9.0)
...
export default function Page() {
  const [input, setInput] = useState<any>({ produtoNome: '', preco: undefined, linkCheckout: '' })
  ...
  async function gerar() {
    ...
    setResult(null)
    try {
      const res = await fetch('/api/agents/carrinhoabandonado', {
        method: 'POST',
  ...
  return (
    <AgentShell
      ...
      tagline="Sequência de recuperação de vendas"
    ...
    >
      ...
      <Field label="Link do checkout">
        <input type="text" value={input.linkCheckout || ''} onChange={e => setInput({ ...input, linkCheckout: e.target.value })} placeholder="" className={inputClass} />
      </Field>
    </AgentShell>
  )
}
