'use client'
// Página auto-gerada — Copy de Lançamento
...
export default function Page() {
  const [input, setInput] = useState<any>({ produtoNome: undefined, nicho: undefined, dor: undefined })
  ...
  async function gerar() {
    ...
    try {
      const res = await fetch('/api/agents/copysniper', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(input) })
  ...
  return (
    <AgentShell
      titulo="Copy de Lançamento" tagline="Emails e mensagens que abrem carteira"
      icone={<span className="text-2xl">🎯</span>} cor="from-red-500 to-rose-700"
    ...
    >
      <Field label="Produto">
        <input type="text" value={input.produtoNome ?? ''} onChange={e => setInput({ ...input, produtoNome: e.target.valueAsNumber || Number(e.target.value) || e.target.value })} placeholder="" className={inputClass} />
      </Field>
      <Field label="Nicho">
        <input type="text" value={input.nicho ?? ''} onChange={e => setInput({ ...input, nicho: e.target.valueAsNumber || Number(e.target.value) || e.target.value })} placeholder="" className={inputClass} />
      </Field>
      <Field label="Dor principal do cliente">
        <input type="text" value={input.dor ?? ''} onChange={e => setInput({ ...input, dor: e.target.valueAsNumber || Number(e.target.value) || e.target.value })} placeholder="" className={inputClass} />
      </Field>
    </AgentShell>
  )
}
