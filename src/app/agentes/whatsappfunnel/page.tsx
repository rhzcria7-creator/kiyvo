'use client'
// Página do agente: Funil de WhatsApp
...
export default function AgentePage() {
  ...
  const formId = 'form-whatsappfunnel'
  ...
  async function handleGerar(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const res = await fetch('/api/agents/whatsappfunnel', {
        method: 'POST',
  ...
  const icon = <MessageCircle className="w-7 h-7" />

  return (
    <AgentShell
      titulo="Funil de WhatsApp"
      tagline="Sequência completa de mensagens WhatsApp que converte em 5-7 dias."
      icone={icon}
      cor="bg-gradient-to-br from-green-500 to-emerald-600"
