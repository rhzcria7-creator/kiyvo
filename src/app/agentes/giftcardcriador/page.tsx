...
export default function Page() {
  const [input, setInput] = useState<any>({ valor: undefined, bonusPercent: undefined })
  ...
  async function gerar() {
    ...
    try {
      const res = await fetch('/api/agents/giftcardcriador', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(input) })
  ...
  return (
    <AgentShell
      titulo="Criador de Gift Card" tagline="Vale-presente promocional"
      icone={<span className="text-2xl">🎁</span>} cor="from-pink-500 to-rose-600"
    ...
    >
      <Field label="Valor (R$)">
        <input type="number" value={input.valor ?? ''} onChange={e => setInput({ ...input, valor: e.target.valueAsNumber || e.target.value })} placeholder="50" className={inputClass} />
      </Field>
      <Field label="Bônus % (opcional)">
        <input type="number" value={input.bonusPercent ?? ''} onChange={e => setInput({ ...input, bonusPercent: e.target.valueAsNumber || e.target.value })} placeholder="10" className={inputClass} />
      </Field>
    </AgentShell>
  )
}
