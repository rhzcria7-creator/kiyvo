'use client'
// Página do agente: Email de Carrinho Abandonado
import { useState } from 'react'
import { motion } from 'framer-motion'
import { ShoppingCart, CheckCircle, Copy } from 'lucide-react'
...
export default function AgentePage() {
  ...
  const formId = 'form-emailrecuperacao'
  ...
  async function handleGerar(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const res = await fetch('/api/agents/emailrecuperacao', {
        method: 'POST',
  ...
  const icon = <ShoppingCart className="w-7 h-7" />

  return (
    <AgentShell
      titulo="Email de Carrinho Abandonado"
      tagline="3 emails que recuperam 15-30% dos carrinhos."
      icone={icon}
      cor="bg-gradient-to-br from-lime-500 to-green-700"
    ...
    >
      <form id={formId} onSubmit={handleGerar} className="space-y-4">
        <Field label="Produto"><input className={inputClass} name="produto" placeholder="" required /></Field>
        <Field label="Preço (R$)"><input className={inputClass} type="number" step="0.01" name="preco" defaultValue="" required /></Field>
        <Field label="Desconto último email (%)"><input className={inputClass} type="number" step="0.01" name="desconto" defaultValue="10" required /></Field>
        <Field label="Tipo de produto"><select className={selectClass} name="segmento" required>
