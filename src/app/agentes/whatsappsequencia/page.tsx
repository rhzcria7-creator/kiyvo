'use client'
// Página do agente: Sequência WhatsApp de Vendas
...
export default function AgentePage() {
  ...
  const formId = 'form-whatsappsequencia'
  ...
  async function handleGerar(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const res = await fetch('/api/agents/whatsappsequencia', {
        method: 'POST',
  ...
  const icon = <MessageSquare className="w-7 h-7" />

  return (
    <AgentShell
      titulo="Sequência WhatsApp de Vendas"
      tagline="Fluxo de 7 mensagens que converte 30%+ em resposta."
      icone={icon}
      cor="bg-gradient-to-br from-green-500 to-emerald-700"
    ...
    >
      <form id={formId} onSubmit={handleGerar} className="space-y-4">
        <Field label="Produto/serviço"><input className={inputClass} name="produto" placeholder="" required /></Field>
        <Field label="Preço (R$)"><input className={inputClass} type="number" step="0.01" name="preco" defaultValue="" required /></Field>
        <Field label="Origem do lead"><select className={selectClass} name="origem" required>
        <option value="anuncio">Anúncio</option>
        <option value="organico">Orgânico</option>
        <option value="indicacao">Indicação</option>
        <option value="quiz">Quiz/Isca</option>
      </select></Field>
        <Field label="Tipo de ticket"><select className={selectClass} name="ticket" required>
        <option value="baixo">Até R$100</option>
        <option value="medio">R$100-500</option>
        <option value="alto">R$500+</option>
      </select></Field>
