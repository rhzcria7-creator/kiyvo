'use client'
// Página auto-gerada para o agente LucroMax (KIYVO v9.0)
...
export default function Page() {
  const [input, setInput] = useState<any>({ precoVenda: undefined, custoProduto: undefined, planoVendedor: '', afiliadoPercent: undefined, incluirImpostos: '' })
  ...
  async function gerar() {
    ...
    setResult(null)
    try {
      const res = await fetch('/api/agents/lucromax', {
        method: 'POST',
  ...
  return (
    <AgentShell
      titulo="LucroMax"
      tagline="Calculadora de lucro REAL com taxas e impostos"
    ...
    >
      <Field label="Preço de venda (R$)">
        <input type="number" value={input.precoVenda || ''} onChange={e => setInput({ ...input, precoVenda: e.target.value })} placeholder="" className={inputClass} />
      </Field>
      <Field label="Custo do produto">
        <input type="number" value={input.custoProduto || ''} onChange={e => setInput({ ...input, custoProduto: e.target.value })} placeholder="0" className={inputClass} />
      </Field>
      ...
      <Field label="Comissão afiliado %">
        <input type="number" value={input.afiliadoPercent || ''} onChange={e => setInput({ ...input, afiliadoPercent: e.target.value })} placeholder="0" className={inputClass} />
      </Field>
      <Field label="Incluir impostos (Simples)">
          <select value={input.incluirImpostos || ''} onChange={e => setInput({ ...input, incluirImpostos: e.target.value })} className={selectClass}>
            <option value="">Selecione...</option>
