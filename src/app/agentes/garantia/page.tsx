'use client'
// Página auto-gerada para o agente Garantia Express (KIYVO v9.0)
...
export default function Page() {
  const [input, setInput] = useState<any>({ produtoNome: '', nicho: '', tipo: '' })
  ...
  async function gerar() {
    ...
    setResult(null)
    try {
      const res = await fetch('/api/agents/garantia', {
        method: 'POST',
  ...
  return (
    <AgentShell
      titulo="Garantia Express"
      tagline="Garantias que reduzem objeções"
      icone={<span className="text-2xl">{ICON}</span>}
      cor="bg-gradient-to-br from-blue-600 to-cyan-700"
      onGerar={gerar}
      loading={loading}
    ...
    >
      ...
      <Field label="Nicho">
        <input type="text" value={input.nicho || ''} onChange={e => setInput({ ...input, nicho: e.target.value })} placeholder="" className={inputClass} />
      </Field>
      <Field label="Tipo de garantia">
          <select value={input.tipo || ''} onChange={e => setInput({ ...input, tipo: e.target.value })} className={selectClass}>
            <option value="">Selecione...</option>
            <option value="incondicional">incondicional</option>
            <option value="resultado">resultado</option>
            <option value="estendida">estendida</option>
            <option value="risco_zero">risco_zero</option>
          </select>
        </Field>
    </AgentShell>
  )
