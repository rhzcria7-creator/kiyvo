'use client'
// Página do agente: Campanha KD Points
import { useState } from 'react'
import { motion } from 'framer-motion'
import { Gem, CheckCircle, Copy } from 'lucide-react'
...
export default function AgentePage() {
  ...
  const formId = 'form-kdpointscampaign'
  ...
  async function handleGerar(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const res = await fetch('/api/agents/kdpointscampaign', {
        method: 'POST',
  ...
  const icon = <Gem className="w-7 h-7" />

  return (
    <AgentShell
      titulo="Campanha KD Points"
      tagline="Crie campanhas de KD Points para fidelizar clientes e aumentar compras repetidas."
      icone={icon}
      cor="bg-gradient-to-br from-amber-400 to-yellow-500"
