'use client'
// Página auto-gerada para o agente ScarcityPro (KIYVO v9.0)
...
export default function Page() {
  const [input, setInput] = useState<any>({ produtoNome: '', estoque: undefined, preco: undefined })
  ...
  async function gerar() {
    ...
    setResult(null)
    try {
      const res = await fetch('/api/agents/scarcitypro', {
        method: 'POST',
  ...
  return (
    <AgentShell
      titulo="ScarcityPro"
      tagline="Escassez ÉTICA e real (nunca fake)"
    ...
    >
      ...
      <Field label="Vagas/estoque real">
        <input type="number" value={input.estoque || ''} onChange={e => setInput({ ...input, estoque: e.target.value })} placeholder="20" className={inputClass} />
      </Field>
