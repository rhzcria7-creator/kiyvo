'use client'
// Página do agente: Teste de Webhook
...
export default function AgentePage() {
  ...
  async function handleGerar(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const res = await fetch('/api/agents/webhooktest', {
        method: 'POST',
  ...
  const icon = <Plug className="w-7 h-7" />

  return (
    <AgentShell
      titulo="Teste de Webhook"
      tagline="Gere payloads Stripe/Supabase para testar seus webhooks."
      icone={icon}
      cor="bg-gradient-to-br from-slate-600 to-slate-800"
