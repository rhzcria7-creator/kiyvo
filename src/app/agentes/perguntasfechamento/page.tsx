...
export default function Page() {
  const [input, setInput] = useState<any>({ produtoNome: undefined })
  ...
  async function gerar() {
    ...
    try {
      const res = await fetch('/api/agents/perguntasfechamento', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(input) })
  ...
  return (
    <AgentShell
      titulo="12 Perguntas que Fecham Venda" tagline="Perguntas que levam ao SIM"
      icone={<span className="text-2xl">❓</span>} cor="from-fuchsia-500 to-pink-700"
