'use client'
// Página do agente: Gerador de Títulos Virais
import { useState } from 'react'
import { motion } from 'framer-motion'
import { Newspaper, CheckCircle, Copy } from 'lucide-react'
...
export default function AgentePage() {
  ...
  async function handleGerar(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const res = await fetch('/api/agents/tituloviral', {
        method: 'POST',
  ...
  const icon = <Newspaper className="w-7 h-7" />

  return (
    <AgentShell
      titulo="Gerador de Títulos Virais"
      tagline="20 títulos que param o scroll e geram clique."
      icone={icon}
      cor="bg-gradient-to-br from-red-500 to-yellow-500"
    ...
    >
      <form id={formId} onSubmit={handleGerar} className="space-y-4">
        <Field label="Tema do conteúdo"><input className={inputClass} name="tema" placeholder="" required /></Field>
        <Field label="Formato"><select className={selectClass} name="formato" required>
