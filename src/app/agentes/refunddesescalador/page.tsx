...
export default function Page() {
  const [input, setInput] = useState<any>({ produtoNome: undefined, motivo: undefined })
  ...
  async function gerar() {
    ...
    try {
      const res = await fetch('/api/agents/refunddesescalador', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(input) })
  ...
  return (
    <AgentShell
      titulo="Desescalador de Reembolso" tagline="Reduz refund SEM negar devolução"
      icone={<span className="text-2xl">🤝</span>} cor="from-emerald-500 to-green-700"
    ...
    >
      ...
      <Field label="Motivo do reembolso">
        <input type="text" value={input.motivo ?? ''} onChange={e => setInput({ ...input, motivo: e.target.valueAsNumber || e.target.value })} placeholder="" className={inputClass} />
      </Field>
    </AgentShell>
  )
}
