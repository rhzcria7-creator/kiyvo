'use client'
// Página do agente: Roteiro Reels/TikTok 15s
import { useState } from 'react'
import { motion } from 'framer-motion'
import { Clapperboard, CheckCircle, Copy } from 'lucide-react'
...
export default function AgentePage() {
  ...
  const formId = 'form-roteiroreels'
  ...
  async function handleGerar(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const res = await fetch('/api/agents/roteiroreels', {
        method: 'POST',
  ...
  const icon = <Clapperboard className="w-7 h-7" />

  return (
    <AgentShell
      titulo="Roteiro Reels/TikTok 15s"
      tagline="Roteiros prontos de 15 segundos com hook, corpo e CTA."
      icone={icon}
    ...
    >
      <form id={formId} onSubmit={handleGerar} className="space-y-4">
        ...
        <Field label="Objetivo"><select className={selectClass} name="objetivo" required>
        <option value="vender">Vender</option>
        <option value="engajar">Engajar</option>
        <option value="leads">Leads</option>
        <option value="autoridade">Autoridade</option>
      </select></Field>
        <Field label="Produto ou tema"><input className={inputClass} name="produto" placeholder="" required /></Field>
