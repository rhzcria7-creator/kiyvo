'use client'
// Página do agente: Planilha de Gastos Doméstica
...
export default function AgentePage() {
  ...
  const formId = 'form-planilhagastos'
  ...
  async function handleGerar(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const res = await fetch('/api/agents/planilhagastos', {
        method: 'POST',
  ...
  const icon = <Wallet className="w-7 h-7" />

  return (
    <AgentShell
      titulo="Planilha de Gastos Doméstica"
      tagline="Orçamento doméstico com método 50/30/20 e 6 jars."
      icone={icon}
    ...
    >
      <form id={formId} onSubmit={handleGerar} className="space-y-4">
        <Field label="Renda mensal (R$)"><input className={inputClass} type="number" step="0.01" name="renda" defaultValue="" required /></Field>
        <Field label="Dependentes"><input className={inputClass} type="number" step="0.01" name="dependentes" defaultValue="0" required /></Field>
        <Field label="Meta de economia (%)"><input className={inputClass} type="number" step="0.01" name="alvoEconomia" defaultValue="20" required /></Field>
        <Field label="Método"><select className={selectClass} name="metodo" required>
        <option value="503020">50/30/20</option>
        <option value="6jars">6 Jars</option>
