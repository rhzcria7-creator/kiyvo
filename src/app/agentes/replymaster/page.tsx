  ...
  return (
    <div className="bg-[#FAFAFA] dark:bg-[#0B0F1A] min-h-screen py-16">
      <div className="max-w-2xl mx-auto px-6">
        ...
        <motion.h1 initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} className="text-4xl md:text-5xl font-black tracking-tight text-[#0F172A] dark:text-white">{TITLES['replymaster']}</motion.h1>
        <p className="mt-4 text-[#64748B]">{DESCS['replymaster']}</p>
        <div className="mt-10 bg-white dark:bg-white/5 rounded-[2rem] p-8 border border-black/5 dark:border-white/10">
          ...
          <div className="mt-6"><Link href="/anunciar"><ShimmerButton icon={<Sparkles size={14}/>}>Usar no Copiloto</ShimmerButton></Link></div>
        </div>
      </div>
    </div>
  )
