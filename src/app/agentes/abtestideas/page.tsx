'use client'
// Página auto-gerada para o agente AB Test Ideas (KIYVO v9.0)
...
export default function Page() {
  const [input, setInput] = useState<any>({ pagina: '' })
  ...
  async function gerar() {
    ...
    setResult(null)
    try {
      const res = await fetch('/api/agents/abtestideas', {
        method: 'POST',
  ...
  return (
    <AgentShell
      titulo="AB Test Ideas"
      tagline="Testes com maior impacto em conversão"
      icone={<span className="text-2xl">{ICON}</span>}
      cor="bg-gradient-to-br from-blue-400 to-cyan-600"
      onGerar={gerar}
      loading={loading}
    ...
    >
      <Field label="Página">
          <select value={input.pagina || ''} onChange={e => setInput({ ...input, pagina: e.target.value })} className={selectClass}>
