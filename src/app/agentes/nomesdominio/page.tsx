'use client'
// Página auto-gerada para o agente Nomes & Domínios (KIYVO v9.0)
...
export default function Page() {
  const [input, setInput] = useState<any>({ nicho: '' })
  ...
  async function gerar() {
    ...
    setResult(null)
    try {
      const res = await fetch('/api/agents/nomesdominio', {
        method: 'POST',
  ...
  return (
    <AgentShell
      titulo="Nomes & Domínios"
      tagline="Sugira nomes de marca memoráveis"
      icone={<span className="text-2xl">{ICON}</span>}
      cor="bg-gradient-to-br from-teal-500 to-teal-700"
      onGerar={gerar}
      loading={loading}
