'use client'
// Página auto-gerada para o agente FAQ Objetor (KIYVO v9.0)
...
export default function Page() {
  ...
  async function gerar() {
    ...
    setResult(null)
    try {
      const res = await fetch('/api/agents/faqobjetor', {
        method: 'POST',
  ...
  return (
    <AgentShell
      titulo="FAQ Objetor"
      tagline="Anteveja objeções no checkout"
      icone={<span className="text-2xl">{ICON}</span>}
      cor="bg-gradient-to-br from-violet-500 to-violet-700"
      onGerar={gerar}
      loading={loading}
