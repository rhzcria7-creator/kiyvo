'use client'
// Página do agente: Legenda Instagram Autêntica
...
export default function AgentePage() {
  ...
  const formId = 'form-legendainsta'
  ...
  async function handleGerar(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const res = await fetch('/api/agents/legendainsta', {
        method: 'POST',
  ...
  const icon = <Camera className="w-7 h-7" />

  return (
    <AgentShell
      titulo="Legenda Instagram Autêntica"
      tagline="Legendas que parecem escritas por você (não por IA)."
      icone={icon}
      cor="bg-gradient-to-br from-orange-500 to-pink-500"
    ...
    >
      <form id={formId} onSubmit={handleGerar} className="space-y-4">
        <Field label="Tipo de post"><select className={selectClass} name="tipo" required>
        <option value="foto_pessoal">Foto pessoal</option>
        <option value="produto">Produto</option>
        <option value="resultado">Resultado</option>
        <option value="reflexao">Reflexão</option>
        <option value="promocao">Promoção</option>
      </select></Field>
        <Field label="Tema do post"><input className={inputClass} name="tema" placeholder="" required /></Field>
        <Field label="Tom de voz"><select className={selectClass} name="tom" required>
        <option value="amigavel">Amigável</option>
        <option value="profissional">Profissional</option>
        <option value="bemhumorado">Bem-humorado</option>
