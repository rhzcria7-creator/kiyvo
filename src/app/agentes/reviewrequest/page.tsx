'use client'
// Página do agente: Pedido de Review
...
export default function AgentePage() {
  ...
  const formId = 'form-reviewrequest'
  ...
  async function handleGerar(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const res = await fetch('/api/agents/reviewrequest', {
        method: 'POST',
  ...
  const icon = <Star className="w-7 h-7" />

  return (
    <AgentShell
      titulo="Pedido de Review"
      tagline="Emails e WhatsApp que fazem 40% dos clientes deixarem avaliação honestamente."
      icone={icon}
