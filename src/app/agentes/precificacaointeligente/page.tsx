'use client'
// Página auto-gerada para o agente Precificação Inteligente (KIYVO v9.0)
...
export default function Page() {
  const [input, setInput] = useState<any>({ custo: undefined, nicho: '', qualidadade: '' })
  ...
  async function gerar() {
    ...
    setResult(null)
    try {
      const res = await fetch('/api/agents/precificacaointeligente', {
        method: 'POST',
  ...
  return (
    <AgentShell
      ...
      tagline="Preços psicológicos que vendem mais"
      icone={<span className="text-2xl">{ICON}</span>}
      cor="bg-gradient-to-br from-purple-600 to-indigo-700"
      onGerar={gerar}
      loading={loading}
    ...
    >
      ...
      <Field label="Qualidade">
          <select value={input.qualidadade || ''} onChange={e => setInput({ ...input, qualidadade: e.target.value })} className={selectClass}>
