...
function findStoreHandle(vendorId?: string, vendedorNome?: string): string | null {
  if (!vendorId && !vendedorNome) return null
  const s = STORES.find(x => x.id === vendorId || x.name === vendedorNome)
  return s ? s.handle.replace('@','') : null
}
...
export function ProductCard({ produto, index = 0 }: { produto: Product; index?: number }) {
  ...
  // Boost real a partir da store + prop estática
  const isBoostedStore = useBoost(s => s.isBoosted(produto.id))
  const [boosted, setBoosted] = useState<boolean>(Boolean(produto.boost))
  useEffect(() => { setBoosted(Boolean(produto.boost) || Boolean(isBoostedStore)) }, [produto.boost, isBoostedStore])
  // Selo "Taxa Zero" para produtos de vendedores novos com poucas vendas
  const isTaxZero = vendas < 50
  ...
  return (
    <motion.div
      ...
      transition={{ delay: Math.min(index * 0.05, 0.45), duration: 0.5, ease: [0.22, 1, 0.36, 1] }}
    ...
    >
      ...
      >
        {boosted && (
          <motion.div
            aria-hidden
            className="pointer-events-none absolute inset-0 rounded-[1.25rem] sm:rounded-[2rem]"
            animate={{ boxShadow: ['0 0 0 0 rgba(251,191,36,0.4)', '0 0 0 8px rgba(251,191,36,0)', '0 0 0 0 rgba(251,191,36,0)'] }}
        ...
        <div className={`relative aspect-[4/3] w-full bg-gradient-to-br ${gradient} overflow-hidden`}>
          ...
          {/* Badges */}
          <div className="absolute top-2 left-2 right-2 flex items-start justify-between gap-2 flex-wrap">
            <div className="flex flex-wrap gap-1">
              ...
              {isTaxZero && desconto === 0 && (
                <motion.span
                  initial={{ scale: 0.8, opacity: 0 }}
                  whileInView={{ scale: 1, opacity: 1 }}
                  transition={{ delay: Math.min(index * 0.04 + 0.22, 0.55) }}
                  className="bg-emerald-500 text-white text-[9px] sm:text-[10px] font-black uppercase tracking-wider px-2 py-1 rounded-full shadow-lg flex items-center gap-0.5"
                >
                  <Flame className="w-2.5 h-2.5" /> 0%
                </motion.span>
              )}
            </div>
            {boosted && (
              <motion.span
                ...
                className="ml-auto bg-gradient-to-r from-amber-400 to-orange-500 text-white text-[9px] sm:text-[10px] font-black uppercase tracking-wider px-2 py-1 rounded-full flex items-center gap-1 shadow-lg"
              >
            ...
            {produto.vendedor_nome && (
              <p className="mt-1.5 text-[10px] sm:text-[11px] text-slate-500 dark:text-slate-400 font-medium truncate flex items-center gap-1">
                ...
                {(() => {
                  ...
                  if (handle) return <Link href={`/loja/${handle}`} onClick={(e) => e.stopPropagation()} className="inline-flex items-center gap-0.5 hover:underline">{content}</Link>
