...
// Também mescla produtos publicados pelo usuário via /anunciar (localStorage).
...
import { mergeUserProducts } from './MergedProducts'
...
export function FeaturedProductsV2() {
  ...
  useEffect(() => {
    let cancelled = false
    fetch('/api/v1/products?limit=16&ordenar=boost')
      .then((r) => r.json())
      .then((json) => {
        if (cancelled) return
        const fromApi = (json.data || []) as Product[]
        const merged = mergeUserProducts(fromApi)
        setProducts(merged.slice(0, 16))
